...
typedef struct Struct_CIPHERBLOB{
    unsigned short keyid;
    unsigned char key[16];
} CIPHERBLOB, *PCIPHERBLOB;

typedef struct __gen_cr_param
{
    ...
    PECCCIPHERBLOB pbCipherCr; //使给定的公钥对 CR 进行加密的结果
} GEN_CR_PARAM, PGEN_CR_PARAM;

typedef struct __read_write_param
{
    u32 uLba; //要读写的扇区号，从 0 开始
    u8* pbData; //要读入的数据或读取数据的缓存
    u32 uLen; //输入时 pbData 大小或要读取的大小，输出时是实际读写大小，单位是扇区（512 字节）
} RW_PARAM, PRW_PARAM;
